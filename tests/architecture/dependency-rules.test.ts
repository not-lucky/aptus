...
type Layer =
  "domain" | "ports" | "application" | "plugins" | "adapter" | "external";

const ALLOWED: Record<
  Exclude<Layer, "external" | "adapter">,
  ReadonlySet<Layer>
> = {
...
const FORBIDDEN_EXTERNAL =
  /^(?:express|(?:node:)?https?|(?:node:)?http|zod|js-yaml|yaml)$|(?:adapter|transport|openai|anthropic|google|azure|aws)/i;
...
function resolveRelative(sourcePath: string, specifier: string): string {
  ...
  return posix.normalize(
    posix.resolve(posix.dirname(sourcePath.split("\\").join("/")), withoutJs),
  );
}
...
export function classifySource(
  ...
  for (const { specifier } of importedSpecifiers(source)) {
    const targetLayer = specifier.startsWith(".")
      ? layerForPath(resolveRelative(sourcePath, specifier))
      : "external";
    if (
      targetLayer === "external" &&
      !specifier.startsWith(".") &&
      FORBIDDEN_EXTERNAL.test(specifier)
    ) {
      violations.push({
        kind: "forbidden-external",
        message: `${fromLayer} imports forbidden external ${specifier}`,
      });
    } else if (
      fromLayer !== "external" &&
      fromLayer !== "adapter" &&
      !ALLOWED[fromLayer].has(targetLayer)
    ) {
      violations.push({
        kind: "dependency-direction",
        message: `${fromLayer} imports ${targetLayer}`,
      });
    }
  }
  ...
  function inspect(node: ts.Node): void {
    if (ts.isPropertyDeclaration(node)) {
      ...
      if (
        ...
        node.name.text === "instance"
      )
        mutableSingleton = true;
    }
  ...
  if (mutableSingleton)
    violations.push({
      kind: "mutable-singleton",
      message: "exported mutable singleton",
    });
  return violations;
}
...
export function undocumentedExports(
  ...
  function visit(node: ts.Node): void {
    ...
    if (
      ...
      violations.push({
        kind: "missing-tsdoc",
        message: `undocumented export in ${sourcePath}`,
      });
    }
    if (ts.isInterfaceDeclaration(node) && exported) {
      for (const member of node.members)
        if (!hasJsDoc(member))
          violations.push({
            kind: "missing-tsdoc-member",
            message: `undocumented interface member in ${sourcePath}`,
          });
    }
    ts.forEachChild(node, visit);
  }
...
describe("dependency classifiers", () => {
  it("rejects forbidden edges, mutable exports, and missing documentation", () => {
    expect(
      classifySource(
        "/tmp/src/domain/bad.ts",
        'import express from "express";',
      ),
    ).toEqual([
      {
        kind: "forbidden-external",
        message: "domain imports forbidden external express",
      },
    ]);
    expect(
      classifySource(
        "/tmp/src/ports/bad.ts",
        'import { GatewayContext } from "../../src/application/lifecycle.js";',
      ),
    ).toEqual([
    ...
    expect(
      classifySource(
        "/tmp/src/application/bad.ts",
        'import adapter from "../adapter/x.js";',
      ),
    ).toEqual([
    ...
    expect(
      classifySource(
        "/tmp/src/application/bad.ts",
        'export { value } from "../adapter/value.js";',
      ),
    ).toEqual([
      { kind: "dependency-direction", message: "application imports adapter" },
    ]);
    expect(
      classifySource(
        "/tmp/src/application/bad.ts",
        'const load = () => import("../adapter/value.js");',
      ),
    ).toEqual([
      { kind: "dependency-direction", message: "application imports adapter" },
    ]);
    expect(
      classifySource(
        "/tmp/src/application/bad.ts",
        "class Registry { static instance = new Registry(); }",
      ),
    ).toEqual([
    ...
    expect(
      classifySource(
        "/tmp/src/application/ok.ts",
        "class Metadata { static readonly instance = {}; }",
      ),
    ).toEqual([]);
    expect(
      undocumentedExports(
        "/tmp/src/application/bad.ts",
        "export interface Missing { value: string };",
      ),
    ).toEqual([
      {
        kind: "missing-tsdoc",
        message: "undocumented export in /tmp/src/application/bad.ts",
      },
      {
        kind: "missing-tsdoc-member",
        message: "undocumented interface member in /tmp/src/application/bad.ts",
      },
    ]);
  });
});
    ...
    for (const layer of [
      ...
      try {
        ...
        for (const path of paths) {
          ...
          const taskThreePath =
            normalized === "domain/routing.ts" ||
            normalized.startsWith("ports/") ||
            normalized.startsWith("application/");
          if (taskThreePath)
            expect(undocumentedExports(path, source), path).toEqual([]);

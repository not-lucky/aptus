...
test.concurrent("systemClock provides monotonic and wall-clock times", () => {
...
test.concurrent("systemSleeper completes sleep and honors 0ms", async () => {
  await systemSleeper.sleep(0);
  await systemSleeper.sleep(-5);
});

test.concurrent("systemSleeper aborts if signal already aborted", async () => {
  const controller = new AbortController();
  controller.abort(new Error("pre-aborted"));
  await assert.rejects(() => systemSleeper.sleep(100, controller.signal), /pre-aborted/);
});

test.concurrent("systemSleeper aborts when signal fires during sleep", async () => {
  const controller = new AbortController();
...
test.concurrent("systemRandomSource returns numbers in [0, 1)", () => {
  for (let i = 0; i < 50; i++) {
...
test.concurrent("TestClock tracks and advances monotonic and wall time", () => {
...
test.concurrent("TestSleeper records sleep durations and auto-advances clock", async () => {
...
test.concurrent("TestRandomSource returns sequence and loops or resets", () => {

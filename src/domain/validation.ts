...
const ROLES: Record<string, true> = {
  system: true,
  developer: true,
  user: true,
  assistant: true,
...
export function validateRequestId(value: unknown): ValidationResult {
  return typeof value === "string" && REQUEST_ID_PATTERN.test(value)
    ...
    : invalid(
        "invalid_request_id",
        "requestId",
        "Expected a safe ASCII request identifier.",
      );
}
...
export function validateRfc3339Timestamp(value: unknown): ValidationResult {
  ...
  const daysInMonth =
    month >= 1 && month <= 12 ? (monthLengths[month - 1] ?? 0) : 0;
  if (
    day < 1 ||
    day > daysInMonth ||
    hour > 23 ||
    ...
    return invalid(
      "invalid_timestamp",
      "timestamp",
      "Expected a real RFC 3339 date and time.",
    );
  }
  return VALID;
}
...
export function validateBase64Media(
  data: unknown,
  mediaType: unknown,
): ValidationResult {
  const issues: Issue[] = [];
  if (typeof data !== "string" || !BASE64_PATTERN.test(data)) {
    issues.push({
      code: "invalid_media",
      path: "data",
      message: "Expected non-empty standard base64 data.",
    });
  }
  if (typeof mediaType !== "string" || !MEDIA_TYPE_PATTERN.test(mediaType)) {
    issues.push({
      code: "invalid_media",
      path: "mediaType",
      message: "Expected a valid media type.",
    });
  }
...
export function validateToolCallArgumentsJson(
  value: unknown,
): ValidationResult {
  if (typeof value !== "string") {
    return invalid(
...
function optionalEnum(
  ...
  if (hasOwn(object, key) && values[String(object[key])] !== true) {
    issues.push({
      code: "invalid_content_block",
      path: `${path}.${key}`,
      message: "Expected a supported literal value.",
    });
  }
}
...
function validateCitation(value: unknown, path: string, issues: Issue[]): void {
  if (!objectValue(value)) {
    issues.push({
      code: "invalid_content_block",
      path,
      message: "Expected a citation object.",
    });
    return;
  }
  const kinds: Record<string, true> = {
    ...
    page_span: true,
    block_span: true,
    search_result_span: true,
    url: true,
    file: true,
  };
  if (kinds[String(value["kind"])] !== true)
    issues.push({
      code: "invalid_content_block",
      path: `${path}.kind`,
      message: "Expected a supported citation kind.",
    });
  for (const key of ["sourceId", "sourceTitle", "citedText"] as const)
    optionalType(value, key, "string", path, issues);
  for (const key of [
    "startIndex",
    "endIndex",
    "pageStart",
    ...
    if (hasOwn(value, key) && typeof value[key] !== "number")
      issues.push({
        code: "invalid_content_block",
        path: `${path}.${key}`,
  ...
  if (hasOwn(value, "raw") && !isJsonValue(value["raw"]))
    issues.push({
      code: "invalid_content_block",
      path: `${path}.raw`,
...
function validateBlockBase(
  ...
  optionalEnum(
    value,
    "status",
    { in_progress: true, completed: true, incomplete: true },
    path,
    issues,
  );
  const cache = value["cacheBreakpoint"];
  if (hasOwn(value, "cacheBreakpoint")) {
    if (!objectValue(cache))
      issues.push({
        code: "invalid_content_block",
        path: `${path}.cacheBreakpoint`,
        message: "Expected a cache breakpoint object.",
      });
    else
      optionalType(cache, "ttl", "string", `${path}.cacheBreakpoint`, issues);
  }
  const metadata = value["providerMetadata"];
  if (
    hasOwn(value, "providerMetadata") &&
    (!objectValue(metadata) || !isJsonValue(metadata))
  )
    issues.push({
      code: "invalid_content_block",
...
export function validateContentBlock(
  ...
  if (!objectValue(value) || typeof value["type"] !== "string") {
    return invalid(
      "invalid_content_block",
      path,
      "Expected a discriminated content block object.",
    );
  }
  const issues: Issue[] = [];
  ...
  switch (type) {
    ...
    case "generated_image":
      ...
      optionalEnum(
        value,
        "background",
        { transparent: true, opaque: true, auto: true },
        path,
        issues,
      );
      break;
    ...
    case "server_tool_result": {
      ...
      if (!Array.isArray(content)) {
        issues.push({
          code: "invalid_content_block",
          path: `${path}.content`,
          message: "Expected a content block array.",
        });
      } else {
    ...
    case "server_tool_call":
      ...
      if (hasOwn(value, "input") && !isJsonValue(value["input"]))
        issues.push({
          code: "invalid_content_block",
          path: `${path}.input`,
          message: "Expected a JSON value.",
    ...
    default:
      issues.push({
        code: "invalid_content_block",
        path: `${path}.type`,
        message: "Unknown content block type.",
      });
  }
  return finish(issues);
}
...
function validateExtensions(value: unknown, path: string): ValidationResult {
  if (!objectValue(value))
    return invalid("invalid_extension", path, "Expected an extension object.");
  const issues: Issue[] = [];
  ...
  if (hasOwn(value, "protocols")) {
    if (!objectValue(protocols)) {
      issues.push({
        ...
        path: `${path}.protocols`,
        message: "Expected a protocol extension record.",
      });
    } else {
      for (const [name, entry] of Object.entries(protocols)) {
        ...
        if (!objectValue(entry)) {
          issues.push({
            code: "invalid_extension",
            path: entryPath,
            message: "Expected a protocol parameter set.",
          });
          continue;
        }
        ...
        if (!validateStringRecord(entry["headers"]))
          issues.push({
            code: "invalid_extension",
            path: `${entryPath}.headers`,
            message: "Expected a string header record.",
          });
        if (
          !Array.isArray(entry["sourceFields"]) ||
          !entry["sourceFields"].every((field) => typeof field === "string")
        )
          issues.push({
            code: "invalid_extension",
            path: `${entryPath}.sourceFields`,
  ...
  if (hasOwn(value, "providers")) {
    if (!objectValue(providers)) {
      issues.push({
        code: "invalid_extension",
        path: `${path}.providers`,
        message: "Expected a provider extension record.",
      });
    } else {
      for (const [name, entry] of Object.entries(providers)) {
        ...
        if (!objectValue(entry)) {
          issues.push({
            code: "invalid_extension",
            path: entryPath,
            message: "Expected a provider parameter set.",
          });
          continue;
        }
        if (typeof entry["provider"] !== "string")
          issues.push({
            ...
            message: "Expected a provider name.",
          });
        if (!objectValue(entry["body"]) || !isJsonValue(entry["body"]))
          issues.push({
            code: "invalid_extension",
  ...
  if (
    hasOwn(value, "custom") &&
    (!objectValue(value["custom"]) || !isJsonValue(value["custom"]))
  ) {
    issues.push({
      code: "invalid_extension",
      path: `${path}.custom`,
...
export function validateCanonicalRequest(value: unknown): ValidationResult {
  if (!objectValue(value)) {
    return invalid(
      "invalid_canonical_request",
      "request",
      "Expected a canonical request object.",
    );
  }
  const issues: Issue[] = [];
  ...
  if (!timestamp.valid)
    issues.push(
      ...timestamp.issues.map((issue) => ({ ...issue, path: "receivedAt" })),
    );
  if (typeof value["model"] !== "string" || value["model"].length === 0) {
    issues.push({
      code: "invalid_canonical_request",
      path: "model",
      message: "Expected a non-empty model name.",
    });
  }
  ...
  if (!objectValue(source)) {
    issues.push({
      code: "invalid_canonical_request",
      path: "source",
      message: "Expected a source object.",
    });
  } else {
    ...
    if (typeof source["path"] !== "string" || source["path"].length === 0)
      issues.push({
        code: "invalid_canonical_request",
        path: "source.path",
        message: "Expected a non-empty source path.",
      });
  }
  ...
  if (!Array.isArray(messages) || messages.length === 0) {
    issues.push({
      code: "invalid_canonical_request",
      path: "messages",
      message: "Expected a non-empty message array.",
    });
  } else {
    messages.forEach((message, messageIndex) => {
      ...
      if (ROLES[String(message["role"])] !== true)
        issues.push({
          code: "invalid_canonical_request",
          path: `${messagePath}.role`,
          message: "Expected a canonical message role.",
        });
      if (
        ...
        issues.push({
          code: "invalid_canonical_request",
          path: `${messagePath}.content`,
          message: "Expected a non-empty content block array.",
        });
      } else {
  ...
  if (!objectValue(routing)) {
    issues.push({
      code: "invalid_canonical_request",
      path: "routing",
      message: "Expected a routing object.",
    });
  } else {
    for (const key of ["modelAlias", "overrideRoute"] as const) {
      if (hasOwn(routing, key) && typeof routing[key] !== "string")
        issues.push({
          code: "invalid_canonical_request",
          path: `routing.${key}`,
          message: "Expected a string.",
    ...
    for (const key of ["maxCostUsd", "maxLatencyMs"] as const) {
      if (
        hasOwn(routing, key) &&
        (typeof routing[key] !== "number" || !Number.isFinite(routing[key]))
      )
        issues.push({
          code: "invalid_canonical_request",
          path: `routing.${key}`,
    ...
    if (hasOwn(routing, "dryRun") && typeof routing["dryRun"] !== "boolean")
      issues.push({
        code: "invalid_canonical_request",
        path: "routing.dryRun",
        message: "Expected a boolean.",
  ...
  if (typeof value["stream"] !== "boolean")
    issues.push({
      code: "invalid_canonical_request",
      path: "stream",
      message: "Expected a stream boolean.",
    });
  if (hasOwn(value, "extensions"))
    appendResult(issues, validateExtensions(value["extensions"], "extensions"));
  return finish(issues);
}

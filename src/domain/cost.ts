...
export function cacheWriteTokens(usage: TokenUsage): number {
  return (usage.cacheWriteBreakdown ?? []).reduce((total, entry) => total + entry.tokens, 0);
}
...
export function zeroCost(): CostMetrics {
  return { inputUsd: 0, outputUsd: 0, cacheReadUsd: 0, cacheWriteUsd: 0, totalUsd: 0, currency: "USD" };
}
 ...
 */
export function calculateCost(usage: TokenUsage, prices: PricesPerMillionUsd): CostResult {
  const cachedInput = usage.cachedInputTokens ?? 0;
  const cacheWrite = cacheWriteTokens(usage);
  const tokenCounts = [usage.inputTokens, usage.outputTokens, cachedInput, cacheWrite];
  for (const count of tokenCounts) {
    if (count < 0) return { ok: false, reason: "negative_tokens" };
    if (!Number.isSafeInteger(count)) return { ok: false, reason: "unsafe_token_count" };
  }
  const priceValues = [prices.input, prices.output, prices.cacheRead, prices.cacheWrite];
  for (const price of priceValues) {
    if (!Number.isFinite(price)) return { ok: false, reason: "non_finite_price" };
    if (price < 0) return { ok: false, reason: "negative_price" };
  }

  const inputUsd = (billableInputTokens(usage) * prices.input) / TOKENS_PER_MILLION;
  ...
  if (![inputUsd, outputUsd, cacheReadUsd, cacheWriteUsd, totalUsd].every(Number.isFinite)) {
    return { ok: false, reason: "non_finite_cost" };
  }
  return { ok: true, cost: { inputUsd, outputUsd, cacheReadUsd, cacheWriteUsd, totalUsd, currency: "USD" } };
}

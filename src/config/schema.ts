...
const SecretRef = z
  .string()
  .regex(
    /^(env|secretmanager|file):[^\s]+$/,
    "secretRef must name a resolver and reference",
  );
...
const Limits = z.object({
  rpm: PositiveInt,
  tpm: PositiveInt,
  dailyTokens: PositiveInt,
  dailyCostUsd: NonNegative,
});
const Server = z
  .object({
...
const Client = z.object({
  id: Id,
  tokenHashRef: SecretRef,
  limits: Limits,
  allowedModelAliases: z.array(Id).min(1),
...
const Provider = z
  ...
  .superRefine((value, context) => {
    if (
      new Set(value.credentials.map((credential) => credential.id)).size !==
...
const Target = z.object({
  providerId: Id,
  physicalModel: NonEmpty,
  pricesPerMillionUsd: Prices,
  capabilities: z.array(Capability),
  contextTokens: PositiveInt,
  defaults: z.record(z.string(), JsonValue).optional(),
});
const Model = z
  .object({ alias: Id, targets: z.array(Target).min(1) })
  .superRefine((value, context) => {
    if (
      new Set(
        value.targets.map(
          (target) => `${target.providerId}:${target.physicalModel}`,
        ),
...
const AttemptBudget = z.object({
  maxAttempts: PositiveInt,
  maxLatencyMs: PositiveInt,
  maxCostUsd: NonNegative,
});
const StatusPolicy = z
  .object({
    retryable: z.array(z.number().int().min(400).max(599)),
    nonRetryable: z.array(z.number().int().min(400).max(599)),
  })
  .superRefine((value, context) => {
...
const Route = z.object({
  id: Id,
  modelAliases: z.array(Id).min(1),
  orderedCandidates: z.array(Candidate).min(1),
  requiredCapabilities: z.array(Capability),
  conditions: z.array(Condition),
  fallbackGroups: z.array(Id),
...
export const GatewayConfigSchema = z
  ...
  .superRefine((value, context) => {
    ...
    unique(
      value.clients.map((client) => client.id),
      "clients",
    );
    unique(
    ...
    unique(
      value.models.map((model) => model.alias),
      "models",
    );
    unique(
      value.routes.map((route) => route.id),
      "routes",
    );
    unique(
      value.plugins.map((plugin) => plugin.id),
      "plugins",
    );
    ...
    value.clients.forEach((client, clientIndex) =>
      client.allowedModelAliases.forEach((alias) => {
        if (!modelAliases.has(alias))
          context.addIssue({
            code: "custom",
            path: ["clients", clientIndex, "allowedModelAliases"],
    ...
    value.models.forEach((model, modelIndex) =>
      model.targets.forEach((target) => {
        if (!providerIds.has(target.providerId))
          context.addIssue({
            code: "custom",
            path: ["models", modelIndex, "targets"],
      ...
      route.orderedCandidates.forEach((candidate, candidateIndex) => {
        if (!providerIds.has(candidate.providerId))
          context.addIssue({
            ...
            message: `unknown provider ${candidate.providerId}`,
          });
        if (!modelAliases.has(candidate.modelAlias))
          context.addIssue({
            code: "custom",
            path: [
              "routes",
            ...
            message: `unknown candidate model ${candidate.modelAlias}`,
          });
        if (!route.modelAliases.includes(candidate.modelAlias))
          context.addIssue({
            code: "custom",
            path: [
              "routes",
            ...
            message: `candidate model ${candidate.modelAlias} is not enabled by route`,
          });
        const model = value.models.find(
          (item) => item.alias === candidate.modelAlias,
        );
        if (
          model &&
          !model.targets.some(
            (target) => target.providerId === candidate.providerId,
          )
        )
          context.addIssue({
            code: "custom",
            path: ["routes", routeIndex, "orderedCandidates", candidateIndex],
            message: `candidate ${candidate.providerId}/${candidate.modelAlias} must match an actual model target`,
          });
      });
      route.fallbackGroups.forEach((group) => {
        if (!routeIds.has(group))
          context.addIssue({
            code: "custom",
            path: ["routes", routeIndex, "fallbackGroups"],
            message: `unknown fallback route ${group}`,
          });
      });
    });
    value.plugins.forEach((plugin, pluginIndex) =>
      [...(plugin.before ?? []), ...(plugin.after ?? [])].forEach(
        (dependency) => {
          if (!pluginIds.has(dependency))
            context.addIssue({
              code: "custom",
              path: ["plugins", pluginIndex],
              message: `unknown plugin dependency ${dependency}`,
            });
    ...
    const orderingEdges = new Map(
      value.plugins.map((plugin) => [plugin.id, [] as string[]]),
    );
    value.plugins.forEach((plugin) => {
      for (const dependency of plugin.before ?? [])
        if (pluginIds.has(dependency))
          orderingEdges.get(plugin.id)?.push(dependency);
      for (const dependency of plugin.after ?? [])
        if (pluginIds.has(dependency))
          orderingEdges.get(dependency)?.push(plugin.id);
    });
    ...
    const visit = (id: string, path: string[]): void => {
      if (visiting.has(id)) {
        context.addIssue({
          code: "custom",
          path: ["plugins"],
          message: `plugin dependency cycle: ${[...path, id].join(" -> ")}`,
        });
        return;
      }
      if (visited.has(id)) return;
      visiting.add(id);
      for (const next of orderingEdges.get(id) ?? [])
        visit(next, [...path, id]);
      visiting.delete(id);
      visited.add(id);
    };
    ...
    if (
      ...
      context.addIssue({
        code: "custom",
        path: ["plugins"],
        message: "authentication plugin must be enabled",
      });
    }
  });
